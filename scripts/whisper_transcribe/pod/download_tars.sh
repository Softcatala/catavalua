...
# to a local directory on the pod, so the transcription run reads audio via
# a local file seek instead of ~180k individual HTTP range requests.
#
# Identical to scripts/language_id/pod/download_tars.sh — duplicated rather
# than shared so this directory stays self-contained and rsyncable to a pod
